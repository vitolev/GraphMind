...
\title{GraphMind: Efficiently Exploring Multi-Agent LLM Topologies with Graph Neural Networks}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.7]
                \node[draw, circle, fill=blue!20, font=\tiny] (s) at (3,5) {START};
                \node[draw, rectangle, fill=green!20, font=\tiny] (d) at (3,4) {Decompose\_3};
                \node[draw, rectangle, fill=orange!20, font=\tiny] (s1) at (1,3) {Solver};
                \node[draw, rectangle, fill=orange!20, font=\tiny] (s2) at (3,3) {Python};
                \node[draw, rectangle, fill=orange!20, font=\tiny] (s3) at (5,3) {Solver};
                \node[draw, rectangle, fill=purple!20, font=\tiny] (c) at (3,2) {Combine};
                \node[draw, rectangle, fill=yellow!20, font=\tiny] (v) at (3,1) {Validator};
                \node[draw, rectangle, fill=orange!20, font=\tiny] (sr) at (1.5,0) {Solver};
                \node[draw, circle, fill=blue!20, font=\tiny] (e) at (3,-1) {END};
                ...
                \draw[->, thick] (s3) -- (c);
                \draw[->, thick] (c) -- (v);
                \draw[->, thick] (v) -- node[right, font=\tiny] {YES} (e);
                \draw[->, thick] (v) -- node[above, font=\tiny] {NO} (sr);
                \draw[->, thick] (sr) -- (e);
                
                \node[font=\tiny, align=center] at (6.5, 4) {Depth = 2\\Nodes = 9};
            \end{tikzpicture}
            
            \vspace{0.3cm}
            \centerline{\small Example generated graph}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item \textbf{Output:} Score ∈ [0,1] via sigmoid
            \end{itemize}
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.85]
                ...
                \node[draw, circle, fill=blue!30, font=\tiny, minimum size=0.5cm] (n1) at (0,3.5) {Solv};
                \node[draw, circle, fill=green!30, font=\tiny, minimum size=0.5cm] (n2) at (1,4) {Extr};
                \node[draw, circle, fill=orange!30, font=\tiny, minimum size=0.5cm] (n3) at (1,3) {Dec};
                \node[draw, circle, fill=purple!30, font=\tiny, minimum size=0.5cm] (n4) at (2,3.5) {Comb};
                ...
                % Virtual node (further right, larger)
                \node[draw, circle, fill=red!30, thick, font=\small, minimum size=0.9cm] (vn) at (5.5,3.5) {Virtual\\Node};
                
                % Virtual node connections (dashed red lines to all graph nodes)
                \draw[<->, dashed, thick, red] (vn) -- (n1);
                \draw[<->, dashed, thick, red] (vn) -- (n2);
                \draw[<->, dashed, thick, red] (vn) -- (n3);
                \draw[<->, dashed, thick, red] (vn) -- (n4);
                
                % Label for graph nodes
                \node[font=\scriptsize, align=center] at (1, 2) {Agent\\Graph};
                
                % Message passing layers annotation
                \node[font=\scriptsize, align=center] at (3.5, 2.5) {Message\\Passing\\Layers};
                \draw[->, very thick, blue] (3.5, 2.2) -- (3.5, 1.5);
                
                % Virtual node embedding (below virtual node)
                \node[draw, rectangle, fill=red!20, font=\small, minimum width=1.4cm, minimum height=0.55cm] (emb) at (5.5, 1) {Virtual\\Embedding};
                \draw[->, very thick] (vn) -- (emb);
                
                % MLP (below embedding)
                \node[draw, rectangle, fill=yellow!30, font=\small, minimum width=1.4cm, minimum height=0.5cm] (mlp) at (5.5, -0.1) {MLP};
                \draw[->, very thick] (emb) -- (mlp);
                
                % Output (lower position)
                \node[draw, circle, fill=orange!40, font=\small] (out) at (5.5, -1.6) {0.87};
                \draw[->, very thick] (mlp) -- (out);
                \node[font=\tiny, align=center] at (5.5, -2.3) {Predicted\\Score};
            \end{tikzpicture}
            
            \vspace{0.2cm}
            \centerline{\small GNN with virtual node architecture}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.7]
                ...
                \draw[->, very thick] (0, 3.5) -- (0, 3);
                \node[font=\tiny] at (1, 3.25) {Sort by score};
                
                % Top candidates - arrow pointing to candidate pool
                \node[draw, rectangle, fill=green!30, minimum width=2.5cm, font=\tiny] (top10) at (-0.5, 2.5) {Top 10 to\\candidate pool};
                
                % Candidate Pool visualization (further to the right)
                \node[draw, rectangle, fill=green!10, minimum width=2.8cm, minimum height=2cm, font=\tiny, thick] (pool) at (4.5, 2.5) {};
                \node[font=\tiny, align=center] at (4.5, 3.8) {\textbf{Candidate Pool}};
                
                % Graphs in pool with GNN scores
                \node[draw, rectangle, fill=blue!20, font=\tiny, minimum width=2.2cm] at (4.5, 3.3) {Graph 1: GNN=0.89};
                \node[draw, rectangle, fill=blue!20, font=\tiny, minimum width=2.2cm] at (4.5, 2.8) {Graph 2: GNN=0.85};
                \node[draw, rectangle, fill=blue!20, font=\tiny, minimum width=2.2cm] at (4.5, 2.3) {Graph 3: GNN=0.82};
                \node[font=\tiny] at (4.5, 1.9) {\vdots};
                \node[draw, rectangle, fill=blue!20, font=\tiny, minimum width=2.2cm] at (4.5, 1.5) {Graph 10: GNN=0.71};
                ...
                \node[draw, rectangle, fill=orange!30, minimum width=2.5cm, font=\tiny] at (-0.5, 1) {Top 5 for evaluation};
                
                \draw[->, very thick] (-0.5, 0.5) -- (-0.5, 0);
                
                % LLM evaluation
                \node[draw, cloud, fill=blue!20, minimum width=2cm, minimum height=1cm, font=\tiny, align=center] at (-0.5, -1) {LLM\\Evaluation};
                
                \node[font=\scriptsize, align=center] at (-0.5, -2.2) {5 math problems\\per graph};
                
                % Cost indicator
                \node[draw, rectangle, fill=red!20, font=\tiny] at (-0.5, -3) {\$ \$ \$ Cost};
            \end{tikzpicture}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.7]
                ...
                \node[font=\scriptsize, align=center] at (3.25, 2.5) {\textbf{Training Data Growth}};
                
                % GNN training process
                \node[draw, rectangle, fill=green!20, minimum width=2cm, font=\tiny] at (1, 3.5) {Graph + Score};
                \draw[->, thick] (2, 3.5) -- (3, 3.5);
                \node[draw, rectangle, fill=blue!20, minimum width=1.5cm, font=\tiny] at (4, 3.5) {GNN};
                \draw[->, thick] (4.75, 3.5) -- (5.5, 3.5);
                \node[font=\tiny] at (6.5, 3.5) {Prediction};
                
                \node[font=\tiny] at (4, 4.2) {MSE Loss};
                \draw[->, thick, red] (6.5, 3.7) .. controls (6.5, 4.5) and (4.75, 4.5) .. (4.75, 3.7);
                \node[font=\tiny] at (5.75, 4.5) {Backprop};
                
                % Improvement arrow
                \draw[->, very thick, green] (0.5, 4.8) -- (6, 4.8);
                \node[font=\scriptsize] at (3.25, 5.2) {Model Improves};
            \end{tikzpicture}
        \end{column}
    \end{columns}
...
\begin{frame}{Random Baseline: Understanding the Distribution}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{itemize}
                \item Two distinct clusters
                \item Good graphs ($>0.5$ accuracy)
                \item Poor graphs ($<0.3$ accuracy)
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \begin{enumerate}
                ...
                \begin{itemize}
                    \item Simple spectral convolution
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.8]
                ...
                \node[font=\scriptsize] at (0, 5) {\textbf{Message Passing Layers}};
                
                % Layer 1
                \node[draw, rectangle, fill=blue!20, minimum width=3cm, minimum height=0.6cm, font=\tiny] (l1) at (0, 4) {Layer 1: Node Features (15 dim)};
                
                \draw[->, thick] (0, 3.7) -- (0, 3.3);
                \node[font=\tiny] at (1.5, 3.5) {Aggregate};
                
                % Layer 2
                \node[draw, rectangle, fill=green!20, minimum width=3cm, minimum height=0.6cm, font=\tiny] (l2) at (0, 3) {Layer 2: Hidden (8-32 dim)};
                
                \draw[->, thick] (0, 2.7) -- (0, 2.3);
                \node[font=\tiny] at (1.5, 2.5) {Aggregate};
                
                % Layer 3
                \node[draw, rectangle, fill=green!20, minimum width=3cm, minimum height=0.6cm, font=\tiny] (l3) at (0, 2) {Layer 3: Hidden (8-32 dim)};
                
                \draw[->, thick] (0, 1.7) -- (0, 1.3);
                \node[font=\tiny] at (1.5, 1.5) {Aggregate};
                
                % Layer 4
                \node[draw, rectangle, fill=green!20, minimum width=3cm, minimum height=0.6cm, font=\tiny] (l4) at (0, 1) {Layer 4: Hidden (8-32 dim)};
                
                \draw[->, thick] (0, 0.7) -- (0, 0.3);
                
                % Virtual node readout
                \node[draw, rectangle, fill=red!20, minimum width=3cm, minimum height=0.6cm, font=\tiny] (vn) at (0, 0) {Virtual Node Embedding};
                
                \draw[->, thick] (0, -0.3) -- (0, -0.7);
                
                % MLP
                \node[draw, rectangle, fill=yellow!20, minimum width=3cm, minimum height=0.6cm, font=\tiny] (mlp) at (0, -1) {MLP + Sigmoid};
                
                \draw[->, thick] (0, -1.3) -- (0, -1.7);
                
                % Output
                \node[draw, circle, fill=orange!30, font=\scriptsize] (out) at (0, -2) {0.87};
            \end{tikzpicture}
        \end{column}
    \end{columns}
...
\begin{frame}{Results: GNN Model Performance}
    \begin{columns}[T]
        ...
        \end{column}
        \begin{column}{0.5\textwidth}
            \includegraphics[width=\textwidth]{../hyperparameter_search/hyperparameter_processing.ipynb}
...
\begin{frame}{Pipeline Dynamics: The Prediction Paradox (2/2)}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \vspace{0.5cm}
            \textbf{Despite this limitation:}
            \begin{itemize}
                \item \alert{Still massively outperforms random search}
                \item 99.9975\% cost reduction maintained
                \item Identifies high-performing topologies
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \centering
            \includegraphics[width=\textwidth]{../logs/analytics/more-agents-real-llm-v4-bestGCN/metric_trends.png}
...
\begin{frame}{Results: GNN-Guided vs Random Search}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            ...
            \\Evaluate 150 instead of 6M graphs
            \\\alert{99.9975\% reduction!}
        \end{column}
        \begin{column}{0.5\textwidth}
            \centering
            ...
            \vspace{0.3cm}
            \small
            Random baseline (gray) vs GNN-guided search (colored)
        \end{column}
    \end{columns}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \begin{tikzpicture}[scale=0.8]
                ...
                \node[draw, star, star points=5, fill=yellow!30, minimum width=2cm, font=\small] at (0, -2) {Solution};
                \node[font=\tiny, align=center] at (0, -2.8) {Balanced\\Sampling};
            \end{tikzpicture}
        \end{column}
    \end{columns}
